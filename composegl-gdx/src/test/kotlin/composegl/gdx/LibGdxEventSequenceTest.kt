...
class LibGdxEventSequenceTest {
    ...
    fun `shift and an arrow selects`() {
        ...
        assertTrue(field.keys.filter { it.key == composegl.ui.input.Key.Right }.all { it.modifiers.shift })
    }
    ...
    fun `a shift left behind by a lost window does not linger`() {
        ...
        assertFalse(field.keys.last { it.key == composegl.ui.input.Key.A }.modifiers.shift)
    }
