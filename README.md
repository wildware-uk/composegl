...
| [Widgets](docs/wiki/Widgets.md) · [Layout](docs/wiki/Layout.md) · [Modifiers](docs/wiki/Modifiers.md) | what there is — buttons, fields, sliders, dropdowns, steppers, dialogs — and how it fits together, down to a panel that shakes on a wrong password, parallax layers that follow the mouse, stick or scroll, panels that animate out before they go, a small thing kept small in a big slot, and borders that are one-sided, dashed or dotted, text lined up by its baseline, sprite-sheet animation, labels players can select and copy, an on-screen keyboard for players with only a pad, and `Modifier.debugBounds()` to see where a widget landed |
