package dev.wildware.composegl.snake.game
...
/**
 * Everything the interface needs to know, in Compose state.
 *
 * This is the seam between the game and the interface: the game loop writes to it, and the
...
class SnakeSession(private val scores: HighScoreStore = HighScoreStore.InMemory()) {
    ...
    /** Cosmetic, and a good excuse for a toggle that changes what OpenGL draws. */
    ...
    /** Space, the pad's Start button, or the button on the panel: one key for both halves. */
    fun togglePause() {
        when (screen) {
    ...
    /** How far through the current step the snake is, 0..1, so the head slides rather than jumps. */
    val stepProgress: Float
        get() = (secondsSinceStep * difficulty.stepsPerSecond).coerceIn(0f, 1f)
