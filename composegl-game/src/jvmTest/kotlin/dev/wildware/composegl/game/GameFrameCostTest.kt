...
/**
 * What a whole HUD built from the game widgets costs when nothing on it is changing.
 *
 * composegl-ui's `FrameCostTest` asks this of a screen of toolkit widgets. The same screen with two
 * health bars and a crosshair on it is asked here, because a bar that animates its trail or a
 * reticle that keeps a clock is exactly the kind of widget that could quietly ask for a frame every
 * frame, or make something per frame, and a per-widget test would not see it add up.
 *
 * The numbers are the toolkit's own, and as loose: an order of magnitude is what is being caught.
 */
class GameFrameCostTest {
