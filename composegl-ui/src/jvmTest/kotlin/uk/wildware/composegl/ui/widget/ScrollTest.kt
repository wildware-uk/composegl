...
class ScrollTest {
    ...
    private fun Spacerish() {
        uk.wildware.composegl.ui.layout.Spacer(Modifier.fillMaxWidth().height(tall / 10f))
    }
    ...
    fun `a list that fits has no bar at all`() {
        ...
        show {
            ScrollArea(Modifier.size(side), state = state) {
                uk.wildware.composegl.ui.layout.Spacer(Modifier.size(50f))
            }
        }
    ...
    fun `contents that shrink pull the scroll back rather than leaving it looking at nothing`() {
        ...
        show {
            ScrollArea(Modifier.size(side), state = state) {
                Column(Modifier.fillMaxWidth()) {
                    repeat(rows) { uk.wildware.composegl.ui.layout.Spacer(Modifier.fillMaxWidth().height(100f)) }
                }
            }
        }
