 ...
 *
 * Made by [measureOverdraw], or `UiTest.overdraw()` in a test, and shaded over the screen by `OverdrawOverlay` in `composegl-debug`.
 */
class OverdrawMap(
 ...
 * letters. Nor is a [DebugOverlay]'s drawing: an overlay's marks are not what the screen paints.
 */
fun measureOverdraw(node: UiNode, like: UiCanvas? = null, cell: Float = 1f): OverdrawMap {
    val root = rootOf(node)
    return measureOverdraw(node, OverdrawMap(root.width, root.height, cell), like)
}

/**
 * [measureOverdraw] into a map already made, cleared first, so an overlay counting every frame makes
 * nothing new while the screen stays the same size. A map smaller than the screen counts the part it
 * covers.
 */
fun measureOverdraw(node: UiNode, into: OverdrawMap, like: UiCanvas? = null): OverdrawMap {
    into.clear()
    OverdrawCanvas(into, like).count(rootOf(node))
    return into
}
